...
async function runInParallel(options, context) {
    const procs = options.commands.map((c) => createProcess(c, options.readyWhen, options.color, calculateCwd(options.cwd, context), options.env ?? {}).then((result) => ({
        result,
        command: c.command,
    })));
...
async function runSerially(options, context) {
    for (const c of options.commands) {
        const success = await createProcess(c, undefined, options.color, calculateCwd(options.cwd, context), options.env ?? {});
        if (!success) {
...
function createProcess(commandConfig, readyWhen, color, cwd, env) {
